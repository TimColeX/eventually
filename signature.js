...
(function (global) {
  ...
  let onDone = null;                     // fired once the splash is fully gone (see play opts)
  function dismiss() {
    ...
    // Tell the caller the splash is gone (the onboarding tour starts here). Fired after the
    // fade so the tour never overlaps the splash, and only ever once.
    if (onDone) { const fn = onDone; onDone = null; setTimeout(fn, 520); }
  }
